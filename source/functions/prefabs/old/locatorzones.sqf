...
while {!_found} do
{
   _truc = Blufor_Truck_Covered createVehicle ([0,0,0]);
   ...
   if (_posDeTrucZ > 0) then {   // SI AU DESSUS DE L'EAU
     _meilleurTruc = Blufor_Truck_Covered createVehicle (_posDeTruc);   // Reference truck
     _posDeMeilleurTruc = getPos _meilleurTruc;
     _playerDistance = _posDeMeilleurTruc distance player;
     deleteVehicle _meilleurTruc;
     
     _trucX = Blufor_Truck_Covered createVehicle ([(_posDeMeilleurTruc select 0)+_radius, _posDeMeilleurTruc select 1, _posDeMeilleurTruc select 2]);
     _trucY = Blufor_Truck_Covered createVehicle ([_posDeMeilleurTruc select 0, (_posDeMeilleurTruc select 1)+_radius, _posDeMeilleurTruc select 2]);
     _truc_X = Blufor_Truck_Covered createVehicle ([(_posDeMeilleurTruc select 0)-_radius, _posDeMeilleurTruc select 1, _posDeMeilleurTruc select 2]);
     _truc_Y = Blufor_Truck_Covered createVehicle ([_posDeMeilleurTruc select 0, (_posDeMeilleurTruc select 1)-_radius, _posDeMeilleurTruc select 2]);
    ...
    if (_diffAbsX<=_diff AND _diffAbsY<=_diff AND _diffAbs_X<=_diff AND _diffAbs_Y<=_diff AND _playerDistance>1000)  then {  // VERIFIE SI RADIUS A NIVEAU (-) ET ELOIGNE DU JOUEUR ET REGARDE LA DISTANCE DES AUTRES ZONES  
     ...
     _null = [format["Zone %1",_i],_points,_zone_radius,_MissionPos,_fortified] spawn duws_fnc_createzone;     
     _found=true;
