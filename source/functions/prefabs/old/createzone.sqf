...
//
//  New usage: _trg = ["Outpost Airbase",5,50,this,true] spawn duws_fnc_createzone
...
_trg setTriggerStatements["this", format["[""%1"",%2,""%3"",""%4""] spawn duws_fnc_captured",_place,_points,_markername,_markername2], ""];
...
if (_fortified) then  
{
      [getpos _trigger] call duws_fnc_createopfortified;
      sleep 2;
};
...
{
      [getpos _trigger, _size] call duws_fnc_createoppatrol;
      sleep (random 3);
      [getpos _trigger, _size] call duws_fnc_createoppatrol;
};
...
if (_size < 251) exitWith  
{
      ...
      [getpos _trigger, _size] call duws_fnc_createoppatrol;
      sleep 2+(random 3);
      [getpos _trigger, _size] call duws_fnc_createopteam
};
